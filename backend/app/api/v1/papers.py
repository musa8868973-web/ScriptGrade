...
async def _resolve_paper(
    db: AsyncSession, student_id: str, user: User
) -> StudentPaper:
...
async def batch_upload(
    ...
    try:
        batch, paper_ids = await ingest_batch(db, exam, current_user, uploads)
...
async def get_paper_detail(
    ...
    paper = await _resolve_paper(db, student_id, current_user)
